...
class MenuModelForm(forms.ModelForm):
    ...
    class Meta:
        ...
        error_messages = {
            ...
            'weight': {'required': '菜单权重不能为空'},
            'icon': {'required': '图标不能为空'}
        }
        widgets = {
            'title': wid.TextInput(attrs={'class': 'form-control'}),
            'weight': wid.NumberInput(attrs={'class': 'form-control'}),
            'icon': wid.RadioSelect(choices=ICON_LIST)
        }


class PermissionModelForm(forms.ModelForm):
    """权限的modelform"""
    parent = forms.ModelChoiceField(queryset=Permission.objects.filter(menu_id__isnull=False), label='父级菜单', required=False)
    class Meta:
        model = Permission
        fields = ['title', 'url', 'url_name', 'icon', 'menu', 'parent']
